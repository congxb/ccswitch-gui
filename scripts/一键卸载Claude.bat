...
title CCSwitch Uninstall
echo.
echo ========================================
echo    CCSwitch - Uninstall Claude Code
echo ========================================
echo.
...
echo [4/5] Check Git for Windows...
if exist "C:\Program Files\Git" (
    set /p UNINSTALL_GIT=       Uninstall Git for Windows? (y/N):
    if /i "!UNINSTALL_GIT!"=="y" (
...
echo [5/5] Check config files...
if exist "%USERPROFILE%\.claude" (
    set /p CLEAN_CLAUDE=       Delete .claude config dir? (y/N):
    if /i "!CLEAN_CLAUDE!"=="y" (
        ...
        echo       .claude dir deleted.
    ) else (
        echo       .claude dir kept.
    )
) else (
    echo       .claude not found, skip.
)
if exist "%USERPROFILE%\.ccswitch" (
    set /p CLEAN_CCSWITCH=       Delete .ccswitch config dir? (y/N):
    if /i "!CLEAN_CCSWITCH!"=="y" (
        ...
        echo       .ccswitch dir deleted.
    ) else (
        echo       .ccswitch dir kept.
    )
) else (
